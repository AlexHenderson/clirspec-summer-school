function [foldinfo, pccvaoutcomes, pccvadata] = kfoldcrossval(data, numfolds, classmembership, classnames, pcs)

% function: kfoldcrossval
%           k-fold cross-validation using PC-CVA
% version:  1.0
%
% [foldinfo, pccvaoutcomes, pccvadata] = kfoldcrossval(data, numfolds, classmembership, classnames, pcs)
%
% where:
...
%function [result, distances, confusion, foldinfo, pccvaoutcomes, pccvadata] = kfoldcrossval(data, numfolds, classmembership, classnames, pcs)
...
[names,counts]=countclasses(classmembership);
maxclassnum=size(names,1);
...
if (~exist('pcs','var'))
    % If not provided, determine the optimal number of principal components 
    % using entire data set
    [cvloadings,cvscores,cvexplained,cvs,pcloadings,pcscores,pcexplained,pcs] = pccva(data,classmembership);
end
...
%         [repoutcome,repdata]=pcdfa(trainingset, trainingnames, trainingmembership, testset, testnames, pcs, 'kfold', 1);
        [repoutcome,repdata]=pccva_test(trainingset, trainingnames, trainingmembership, testset, testnames, pcs, 'kfold', 1);
        firstpass = 0;
    else
        [repoutcome,repdata]=pccva_test(trainingset, trainingnames, trainingmembership, testset, testnames, pcs, 'kfold', 0);
    end
    pccvaoutcomes{rep,1}=repoutcome;
    pccvadata{rep,1}=repdata;
    
%     [dist, nearest, correctlyclassified, cm]=getdist(repoutcome.cvascores, trainingmembership, repoutcome.cvatestvars, testmembership);
