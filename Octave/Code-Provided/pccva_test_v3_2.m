function [outcome, dataused]=pccva_test_v3_2(traindata, trainclassnames, trainclassmembership, testdata, testclassnames, pcs, samplingtype, plotresults)
...
if (size(trainclassnames,2) > 1)
    numtrainclasses=size(unique(trainclassnames, 'rows'), 1);
else
    numtrainclasses=size(unique(trainclassnames), 1);
end
...
if (size(trainclassnames,2) > 1)
    ...
    numtestclasses=size(unique(testclassnames), 1);
end
...
[cvloadings,cvscores,cvexplained,cvs,pcloadings,pcscores,pcexplained,pcs] = pccva(traindata,trainclassmembership,pcs);
...
% [cvscores, dfloadings, cvexplained] = dfa(pcscores, trainclassmembership, dfs);
[cvloadings,cvscores,cvexplained] = cva(pcscores,trainclassmembership);

%% Perform canonical variates analysis
[eigenvectors,eigenvalues,percent_explained_variation] = cva(pcscores,trainclassmembership);

%% Generate output by mapping back to the original data
cvloadings = testdata * pcloadings * eigenvectors;
cvscores = pcscores * (eigenvectors * diag(eigenvalues));
% cvexplained = percent_explained_variation;

projtest=testdata * pcloadings;
% cvprojtest=projtest * eigenvectors * diag(cvexplained);
cvprojtest=projtest * eigenvectors * diag(eigenvalues);

% locate the test data in the new matrix rotation
% projtest=testdata * pcloadings';
% projtest=testdata * pcloadings;
% 
% cvprojtest=projtest * cvloadings * diag(eigenvalues);

% store for output
outcome.pcascores=pcscores;
outcome.pcaloadings=pcloadings;
outcome.pcapercent_explained=pcexplained;
outcome.pcatestvars=projtest;

outcome.cvascores=cvscores;
outcome.cvaloadings=cvloadings;
outcome.cvaeigenvals=cvexplained;
outcome.cvatestvars=cvprojtest;
outcome.cvs=cvs;
...
if exist('plotresults', 'var') && plotresults == 0
    ...
    xlabel(['principal component 1 (', num2str(pcexplained(1),4), '%)']);
    ylabel(['principal component 2 (', num2str(pcexplained(2),4), '%)']);
    ...
    gscatter(pcscores(:,1), pcscores(:,2), trainclassnames, colours, 'o');
    hold on;
    gscatter(projtest(:,1), projtest(:,2), testclassnames, testcolours, 'x'); 
    hold off;
    xlabel(['principal component 1 (', num2str(pcexplained(1),4), '%)']);
    ylabel(['principal component 2 (', num2str(pcexplained(2),4), '%)']);
...
%        edges=linspace(-absmax, absmax+0.00000001,length(cvscores));

        minval = min(cvscores);
        maxval = max(cvscores);
        extrabucket= (maxval-minval)/length(cvscores);
        edges=linspace(minval, maxval+extrabucket,length(cvscores));
        
        [n1,bin1] = histc(cvscores(trainindex1,1),edges);
        [n2,bin2] = histc(cvscores(trainindex2,1),edges);
        n=[n1,n2];
        bar(edges,n,2,'histc');  
        histcolourmap=coloursasmap(1:2, :);
        colormap(histcolourmap);

        xlabel(['canonical variate 1 (eig=', num2str(cvexplained(1),4), ')']);
        ylabel('counts per bin');
        title(['cva training data (', num2str(pcs), ' PCs)']);
        if (ismatlab())
            legend(untrainnames(1,:), untrainnames(2,:),'Location','Best');
        else
            legend(untrainnames(1,:), untrainnames(2,:));
        end      
        
    else
        gscatter(cvscores(:,1), cvscores(:,2), trainclassnames, colours, 'o'); 
        xlabel(['canonical variate 1 (eig=', num2str(cvexplained(1),4), ')']);
        ylabel(['canonical variate 2 (eig=', num2str(cvexplained(2),4), ')']);
        title(['cva training data (', num2str(pcs), ' PCs)']);
        if (ismatlab())
            legend('Location','Best');
        else
...
%         edges=linspace(-absmax,absmax,maxedges);
        ...
        gscatter(cvscores(:,1), cvscores(:,2), trainclassnames, colours, 'o');
        hold on;
        gscatter(cvprojtest(:,1), cvprojtest(:,2), testclassnames, testcolours, 'x');
        hold off;
        xlabel(['canonical variate 1 (eig=', num2str(cvexplained(1),4), ')']);
        ylabel(['canonical variate 2 (eig=', num2str(cvexplained(2),4), ')']);
        title(['cva training and test data (', num2str(pcs), ' PCs)']);
